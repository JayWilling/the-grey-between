...
export function LensFlareEffect(
  ...
  let oldOpacity = oldOpacityValue;
  ...
  const lensFlareMaterial = new THREE.ShaderMaterial({
    uniforms: {
      ...
      secondaryGhosts: { value: false },
      starBurst: { value: false },
      ghostScale: { value: 0.3 },
      aditionalStreaks: { value: false },
      followMouse: { value: false },
      lensDirtTexture: {
        value: new THREE.TextureLoader().load("https://i.ibb.co/c3x4dBy/lens-Dirt-Texture.jpg")
      }
    },
    /*GLSL */
    fragmentShader: `
        ...
        if (intersects[0].object.visible) {
          ...
          if (intersects[0].object.material.transmission) {
            if (intersects[0].object.material.transmission > 0.2) {
              internalOpacity = oldOpacity.value * (intersects[0].object.material.transmission * 0.5);
            } else {
              internalOpacity = 0;
            }
            ...
            if (intersects[0].object.material.transparent) {
              if (intersects[0].object.material.opacity < 0.98) {
                internalOpacity = oldOpacity.value / (intersects[0].object.material.opacity * 10);
              }
            } else {
